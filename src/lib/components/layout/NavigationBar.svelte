<script lang="ts">
  ...
  import {
    mixedColors,
    mixedColorsUnlocked,
  } from "$lib/currency/implementations/MixedColorsCurrency";
  import {
    pureColors,
    pureColorsUnlocked,
  } from "$lib/currency/implementations/PureColorsCurrency";
  import {
    lux,
    formatLux,
    WINNING_GOAL,
  ...
  function getLuxStyling(total: number, intensity: number) {
    const baseClasses =
      "text-2xl font-bold bg-gradient-to-r from-purple-400 via-cyan-400 to-blue-400 bg-clip-text text-transparent transition-all duration-500";
  ...
  function getLuxContainerStyling(total: number, intensity: number) {
    const baseClasses =
      "bg-gradient-to-r backdrop-blur-sm border rounded-lg px-4 py-2 transition-all duration-500";
...
<nav
  class="sticky top-0 bg-black/90 backdrop-blur-sm border-b border-green-500/30 z-50"
>
  <div class="px-4 py-2">
    <div class="flex justify-between items-start">
      ...
      <div class="flex flex-col gap-1 flex-shrink-0">
        ...
        {#if $mixedColorsUnlocked}
          {@const mixedWithCounts = mixedColors
            .getMixedColors()
            .filter((c) => c.count > 0)}
          {#if mixedWithCounts.length > 0}
            <div class="flex items-center gap-3 sm:gap-4">
              {#each mixedWithCounts as color}
                <div class="flex items-center gap-1">
                  <div
                    class="w-2 h-2 rounded-full shadow-lg {color.circleColor} {color.shadowColor}"
                  ></div>
                  <span
        ...
        {#if $pureColorsUnlocked}
          {@const pureWithCounts = pureColors
            .getPureColors()
            .filter((c) => c.count > 0)}
      ...
      <div class="flex-1 text-center">
        <div class="flex items-center justify-center gap-3">
          <div
            class={getLuxContainerStyling(
              $lux.displayTotal,
              lux.getDisplayIntensity(),
            )}
          >
            <span
              class={getLuxStyling(
