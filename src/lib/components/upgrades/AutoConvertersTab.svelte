<script lang="ts">
  ...
  let whiteConverters = $derived(
    convertersList.filter((c) => c.type === "white"),
  );
  let mixedConverters = $derived(
    convertersList.filter((c) => c.type === "mixed"),
  );
  let pureConverters = $derived(
    convertersList.filter((c) => c.type === "pure"),
  );

  let allConvertersPaused = $derived(
    purchasedConverters.length > 0 &&
      purchasedConverters.every((c) => !c.enabled),
  );
...
<div class="h-full flex flex-col">
  <div class="mb-6 text-center">
    <h2
      class="text-2xl font-bold uppercase tracking-wider text-cyan-400 glow-text mb-2"
    >
      Auto Converters
    </h2>
    {#if activeConvertersList.length > 0}
      <div class="text-sm text-green-400 mb-3">
        {activeConvertersList.length} active converter{activeConvertersList.length ===
        1
          ? ""
          : "s"} running
      </div>
    {:else}
      <div class="text-sm text-gray-400 mb-3">No active converters</div>
    {/if}

    {#if purchasedConverters.length > 0}
      <div class="flex justify-center">
        {#if allConvertersPaused}
          <GameButton color="blue" onclick={handleResumeAll} class="px-6 py-2">
        ...
        {:else}
          <GameButton color="red" onclick={handlePauseAll} class="px-6 py-2">
            <div class="flex items-center gap-2">
              <span>⏸️</span>
              <span>Pause All</span>
            </div>
    ...
    <div class="flex-1 flex items-center justify-center">
      <div class="text-center opacity-60 space-y-4">
        ...
        <div>
          ...
          <p class="text-sm">
            Reach 10 lifetime white pixels to unlock your first auto converter!
          </p>
        </div>
      </div>
    </div>
  {:else}
    ...
    <div class="flex-1 overflow-y-auto space-y-8">
      ...
      {#if whiteConverters.length > 0}
        <div>
          <h3
            class="text-lg font-bold uppercase tracking-wide text-white mb-4 opacity-75"
          >
            White Pixel Converters
          </h3>
          <div class="grid grid-cols-1 2xl:grid-cols-2 3xl:grid-cols-3 gap-4">
      ...
      {#if mixedConverters.length > 0}
        <div>
          <h3
            class="text-lg font-bold uppercase tracking-wide text-green-400 mb-4 opacity-75"
          >
      ...
      {#if pureConverters.length > 0}
        <div>
          <h3
            class="text-lg font-bold uppercase tracking-wide text-purple-400 mb-4 opacity-75"
          >
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 text-xs">
        ...
        <div class="bg-white/5 border border-white/20 rounded p-3">
          ...
          <p class="text-gray-300 mb-2">
            Automatically convert RGB → White pixels
          </p>
          <p class="text-gray-400">
            • Uses same cost scaling as manual conversion
          </p>
        ...
        <div class="bg-purple-500/5 border border-purple-500/20 rounded p-3">
          ...
          <p class="text-gray-300 mb-2">
            Auto-convert 3 of same color → Pure color
          </p>
