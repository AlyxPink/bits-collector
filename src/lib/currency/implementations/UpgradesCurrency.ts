...
class UpgradesCurrency extends MultiCurrencyBase<UpgradeState> {
	...
	getAmount(): number {
		// For upgrades, return total powerup levels as a general "amount"
		const state = get(this.store);
		return Object.values(state.powerups).reduce((total, powerup) => total + powerup.level, 0);
	}

	addAmount(): void {
		// Not applicable for upgrade system
		throw new Error("addAmount not applicable for upgrades currency");
	}

	spendAmount(): boolean {
		// Not applicable for upgrade system
		throw new Error("spendAmount not applicable for upgrades currency");
	}

	canAfford(): boolean {
		// Not applicable for upgrade system
		throw new Error("canAfford not applicable for upgrades currency");
	}
	...
	purchaseGenerator(id: string): boolean {
		...
		// Use imported pixels store
		const pixelCount: any = get(pixels);
		...
		if (pixelCount.white >= cost) {
			// Deduct cost
			pixels.update((p: any) => ({ ...p, white: p.white - cost }));
	...
	purchaseTabUnlock(tabId: string): boolean {
		...
		// Use imported stores
		const pixelCount: any = get(pixels);
		const lumenCount: any = get(lumen);
	...
	getPureColorBoostDetails(generatorColor: "red" | "green" | "blue" | "random") {
		const currentMultiplier = this.getPureColorBoost(generatorColor);
	...
	getEffectiveGeneratorRate(id: string): number {
		...
		const pureColorMultiplier = this.getPureColorBoost(upgrade.color);
	...
	processAutoTick(deltaTimeSeconds?: number): void {
		const now = Date.now();
...
export const upgrades = new UpgradesCurrency();
...
// Tab unlock status derived stores - subscribe to all dependencies properly
export const tabUnlockStatus = derived(
	[upgrades, pixels, lumen],
	([$upgrades, $pixels, $lumen]) => {
		const status: Record<
			string,
		...
		// Ensure we have valid store data
		if (!$pixels || !$lumen) {
			return status;
		}
