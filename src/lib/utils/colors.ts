export type ColorVariant = 'red' | 'green' | 'blue' | 'purple' | 'yellow' | 'white' | 'gray';
...
export function getShadowStyle(variant: ColorVariant, intensity: number = 0.3): string {
  const theme = getColorTheme(variant);
  return `box-shadow: 0 0 15px rgba(${theme.shadowColor}, ${intensity})`;
}

export function getHoverShadowStyle(variant: ColorVariant, intensity: number = 0.5): string {
  const theme = getColorTheme(variant);
  return `box-shadow: 0 0 25px rgba(${theme.shadowColor}, ${intensity})`;
}

// Utility for converting upgrade color to ColorVariant
export function upgradeColorToVariant(color: 'red' | 'green' | 'blue' | 'random'): ColorVariant {
